...
```json
{
  ...
  "schemaVersion": "v1",
  "doc": "README.md",
  "summary": { "blocks": 12, "coveredBlocks": 7, "uncoveredBlocks": 5, "coverageRatio": 0.58 },
  "regions": [
    {
...
behavioral verdict, so when any exist the `next` hint leads with them
(`"N uncovered block(s) are executable — record them with --verifier command:"…""`).
`coverage` still only tags — it never records a claim; creation stays explicit.
...
summary: { blocks, coveredBlocks, uncoveredBlocks, coverageRatio }, regions[], next }`
