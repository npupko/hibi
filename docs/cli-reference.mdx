...
`hibi` is a single self-contained executable. Every command prints **JSON by
default** and stays quiet otherwise; add `--pretty` for human-readable output.
Output leads with the decision (`status`, `gates`, `side`) and trails the bulky
evidence, so a truncated read still surfaces the verdict.
