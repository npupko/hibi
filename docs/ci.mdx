---
title: "CI, git hooks & agents"
description: "Wire Hibi into continuous integration, git hooks, and an AI agent's edit loop so stale docs are caught before anyone trusts them."
icon: "robot"
---
...
rewrites prose — it only flags and, optionally, stamps a status banner. This page
shows how to wire that flagging into the three places it earns its keep:
continuous integration, a git hook, and an AI coding agent's edit loop.

The shape is the same everywhere. You run a Hibi command, it emits a verdict, and
you act on its **exit code**: `0` is clean, `2` gates, `3` warns, `1` is an
operational error. Because no model runs in the check loop, the same working tree
always yields the same verdicts — that is what makes "is this doc stale?" a signal
you can gate a pipeline on rather than a probabilistic guess.
...
<Note>
  ...
  flag decides the fix. Suspect, not false.
</Note>
...
tag — `npupko/hibi@v0.1.2` (pin to the latest release), or `@main` to track the
...
it is just a command that exits non-zero on drift, so it drops into whatever you
already use.
...
<Steps>
  ...
  <Step title="Stop — report what an edit invalidated">
    ...
    reports exactly which claims that change invalidated, so the agent can draft
...
<Note>
  ...
  index. A cache would re-introduce exactly the drift Hibi exists to kill: the
...
| `3` | warning: `moved` or `at-risk` — re-anchorable or advisory |
| `1` | operational error |

`moved` and `at-risk` never gate, and `suggested` claims never set a failing exit
code — so a build only breaks on a confirmed, enforced claim whose evidence truly
