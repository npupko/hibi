...
into one flag. This page is the map of all four, plus how a document moves from
`active` to `archived`, and what to do when a claim needs attention.
...
You do not pick `enforced` casually — Hibi earns it for you, and refuses it if
...
replaced wholesale, withdrawn, or moved out of the read path entirely.
...
| `doc:changed` / `doc:ambiguous` | Banner noting the prose diverged; re-confirm or retire the claim. |
| `doc:orphaned` | The documented sentence was deleted — relocate or retire it. |
