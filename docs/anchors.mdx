...
flowchart TD
  ...
  DOC --> DS["text-quote · text-position<br/>inline-id?"]
...
| `ast-node` | code | the enclosing named syntax node | relocation, reformatting | A code symbol, snapped via tree-sitter to the smallest enclosing named node. Code side only — the doc side is format-agnostic and carries no structural selector. |
...
## The record-time quote guard

A doc anchor is only as good as the span it pins, so `record`,
`record --from-file`, and `reanchor` refuse a doc-side quote that cannot anchor
reliably — fixing a bad anchor at birth rather than flagging it forever:

- **Too short.** A quote shorter than 8 characters is too common to locate
  reliably; record a wider span (`--doc-range`) that covers the full sentence.
- **Ambiguous.** A quote that occurs more than once in the document, where the
  48-char context still can't single out one occurrence, is refused; widen the
  span or add an inline ID and re-record.

The exact rejection messages are on the
[CLI reference](/cli-reference#hibi-record).

## The pure-move exception

Reanchoring a `verified` claim without a `--ref` normally downgrades trust to
`inferred` — nobody re-attested that the claim is still *true* (see
[Lifecycle](/lifecycle#reanchoring-is-an-attestation)). One case is exempt: a
**pure move**. When the re-resolved doc quote is byte-identical to the stored one
and re-resolves uniquely at similarity 1.0, **and** the code side re-resolves as
exactly `unchanged`, the claim keeps `verified` trust and records no downgrade. A
byte-shift is evidence-neutral: there is nothing to re-attest. Anything fuzzier —
any similarity below 1.0, any code-side state other than `unchanged`, any doc
text difference — downgrades exactly as before.
...
<Steps>
  ...
  <Step title="Tier 2: structural">
    ...
    enclosing named node, plus a two-tier AST hash — code side only. The doc side
    is format-agnostic: it rests on `text-quote` (48-char context) + `text-position`
    (+ optional `inline-id`), with no structural selector. Grammars: TypeScript,
