---
title: "Anchors & selectors"
description: "How Hibi binds a documented sentence to the code it describes — with redundant selectors fused into a single confidence score."
icon: "anchor"
---
...
<Note>
  ...
  source of truth, so a doc that has been quietly reworded can't hide behind a
...
|---|---|---|---|---|
| `text-quote` | both | the exact text plus a 32-character prefix and suffix | moves, small edits | Matched fuzzily (W3C TextQuoteSelector + Bitap). The base selector on both sides. |
...
Quoted text breaks the moment someone reformats. Syntax nodes move when code is
...
The bundle's confidence is the weighted average over the selectors that actually
resolved:
...
<AccordionGroup>
  <Accordion title="At least two selectors must resolve">
    ...
    match — so Hibi refuses to grade on it.
  </Accordion>
  ...
  <Accordion title="Fuzzy-matching parameters">
    ...
    `0.4`, `Match_Distance` `100000`, with a 32-character context window on each
...
The order matters. Hibi resolves the **doc** side first: it finds the sentence in
...
<Steps>
  ...
  <Step title="Tier 3 — behavioral">
    ...
    ascending") need a different mechanism — change-gated attention and executable
    verifiers, still with no model on the verdict path.
  </Step>
</Steps>
