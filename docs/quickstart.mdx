...
Hibi ships as a single self-contained executable — no runtime to install. You can
also add it to a Bun project from npm.
...
just invalidated. Both exit `2` on a gating problem and `3` on a warning, so they
