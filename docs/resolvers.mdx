...
<ParamField path="verify" type="method">
  Params: `{ assertion, verifier, changedEvidence }`. No file contents are sent. Returns `{ behavior: "supported" | "refuted" }`. An error response, a timeout, or a spawn failure counts as no result.
</ParamField>
