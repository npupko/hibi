---
title: "Resolvers"
description: "Extend Hibi's drift detection with out-of-process resolvers that speak one JSONL-RPC protocol."
icon: "puzzle-piece"
---

A **resolver** is an external program that does the actual work of grading an
anchor or running a verifier. Hibi's core is deliberately tiny — "if it isn't
core, it's a resolver or a consumer" — so even the built-in drift and
...
default**: the resolvers that produce verdicts are pure graders fed exactly the
...
file in a directory can never silently start running code in your check loop.
...
verifier, dispatched through `verify`, is what can actually confirm or refute the
