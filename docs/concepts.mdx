---
title: "How it works"
description: "Claims, redundant anchors, and the doc-first flow Hibi uses to grade drift — deterministically, with no model in the loop."
...
This page builds the mental model, then the mechanics, then the doc-first flow that ties
them together.
...
<Info>
  ...
  the evidence under a claim moved — the human or agent decides what to do about it. Suspect,
  not false.
</Info>
...
point at the same span, so no single edit can silently break the link:
...
- an optional **`inline-id`** marker on docs you own that aids re-anchoring but never restates
  the claim — if the marker and the prose disagree, the prose wins;
...
Keeping the two apart is deliberate. The code under a claim can move without the documented
...
Here is the path a claim takes from the moment you record it to the verdict and exit code.
Notice that Hibi resolves the **doc side first**.
...
<Steps>
  <Step title="Record">
    ...
    per claim — and become the baseline. The anchor *is* the baseline, so `check` is fully
    offline and correct under a shallow clone.
  </Step>
  <Step title="Resolve the doc side">
    ...
    `doc:changed` and Hibi stops — it will not verify a sentence that no longer says what it
    used to against the code as if nothing happened.
  </Step>
  ...
  <Step title="Grade">
    That confidence is graded into a `code:…` resolution state by fixed bands. This is the
    structural verdict for the code side.
  </Step>
  <Step title="Behavioral routing">
    If the claim is behavioral, a change-gate decides whether reachable evidence actually
