...
import { supersede, retract, amendedPropositions } from "../src/engine/supersede.ts";
...
describe("supersession edges: forward-authored, reverse-derived (§4, §6)", () => {
  test("supersedes (full) flips the old doc to superseded and derives the reverse edge", async () => {
    const r = await repo();
    const { newDoc, oldDoc } = await supersede(r.store, { newDocPath: "v2.md", oldDocPath: "v1.md", type: "supersedes" });
    expect(newDoc.edges).toContainEqual({ type: "supersedes", target: oldDoc.id, derived: false });
    expect(oldDoc.edges).toContainEqual({ type: "superseded-by", source: newDoc.id, derived: true });
  ...
  test("amends (partial) keeps the doc active-but-amended and flips named propositions", async () => {
    ...
    const { oldDoc } = await supersede(r.store, {
      newDocPath: "v2.md", oldDocPath: "v1.md", type: "amends", propositions: ["prop_a", "prop_b"],
    });
    expect(oldDoc.lifecycle).toBe("amended");
    expect([...amendedPropositions(oldDoc)].sort()).toEqual(["prop_a", "prop_b"]);
  });

  test("amends requires proposition ids", async () => {
    const r = await repo();
    await expect(supersede(r.store, { newDocPath: "v2.md", oldDocPath: "v1.md", type: "amends" })).rejects.toThrow();
  });

  test("edges are idempotent (re-authoring does not duplicate)", async () => {
    const r = await repo();
    await supersede(r.store, { newDocPath: "v2.md", oldDocPath: "v1.md", type: "supersedes" });
    await supersede(r.store, { newDocPath: "v2.md", oldDocPath: "v1.md", type: "supersedes" });
...
describe("supersession and code-drift are surfaced together (§4, §6)", () => {
  test("an amended doc with a separately-drifted claim shows both in the banner", async () => {
    ...
    const a = await record(r, { doc: "doc.md", text: "A is one", file: "src/a.ts", quote: "A = 1" });
    const b = await record(r, { doc: "doc.md", text: "B is two", file: "src/a.ts", quote: "B = 2" });

    // Amend proposition A via a newer doc.
    await supersede(r.store, { newDocPath: "newer.md", oldDocPath: "doc.md", type: "amends", propositions: [a.proposition.id] });
