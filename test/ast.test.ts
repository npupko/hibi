...
describe("tree-sitter snapping & two-tier hash (§17.2)", () => {
  test("structural hash is invariant under re-indentation", () => {
    ...
    const fb = analyzer.analyze(b, "typescript", region(b, "MAX_ATTEMPTS   =   5"))!;
  ...
  test("a rename keeps the structural hash but changes the semantic hash", () => {
    ...
    const fa = analyzer.analyze(a, "typescript", region(a, "retry(maxAttempts"))!;
...
describe("value extraction (§17.4)", () => {
  test("extracts a scalar number", () => {
    const t = "const MAX = 5;";
    expect(analyzer.extractValue(t, "typescript", region(t, "MAX = 5"))).toBe("5");
  });
  test("extracts a string", () => {
    const t = 'const name = "alice";';
    expect(analyzer.extractValue(t, "typescript", region(t, 'name = "alice"'))).toBe('"alice"');
  });
  test("extracts an array with whitespace stripped", () => {
    const t = "const xs = [1, 2, 3];";
    expect(analyzer.extractValue(t, "typescript", region(t, "xs = [1, 2, 3]"))).toBe("[1,2,3]");
  });
  test("Python integer", () => {
    const t = "MAX = 5";
  ...
  test("Rust integer literal", () => {
    ...
    expect(analyzer.extractValue(t, "rust", region(t, "MAX: u32 = 5"))).toBe("5");
  });
  test("Go int literal", () => {
    const t = "const Max = 5";
...
describe("end-to-end value veto: a 5 → 50 change trips even at the boundary (§4, §17.3)", () => {
  test("value selector catches a boundary insertion the text tier misses", () => {
    ...
    const anchor = buildAnchor("src/retry.ts", original, region(original, "MAX_ATTEMPTS = 5"), {
      language: "typescript",
      analyzer,
    });
    ...
    const assertion: Assertion = {
      id: "asrt_1", propositionId: "p", documentId: "d", owner: "x", ref: "r",
      anchor, attrs: {},
    };
  ...
  test("an unchanged file grades fresh with full corroboration", () => {
    ...
    const assertion: Assertion = {
      id: "a", propositionId: "p", documentId: "d", owner: "x", ref: "r", anchor, attrs: {},
    };
